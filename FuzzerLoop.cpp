...
void Fuzzer::PrintStats(const char *Where, const char *End, size_t Units,
  ...
  Printf(" states: %zd", SM_num);
  Printf(" leaves: %zd", SM_root->get_num_leaves());
  Printf("%s", End);
}
...
static void WriteToExecutionLog(const std::string &ExecutionLogFile, 
                                const uint8_t *Data, size_t Size) {
  if (ExecutionLogFile.empty())
    return;
  AppendToFile(Data, Size, ExecutionLogFile);
  AppendToFile((uint8_t *)"\n----\n", 6, ExecutionLogFile);
...
bool Fuzzer::RunOne(const uint8_t *Data, size_t Size, bool MayDeleteFile,
  ...
  WriteToExecutionLog(Options.ExecutionLogFile, Data, Size);
  ...
  if (NumNewFeatures || SM_added_states || ForceAddToCorpus) {
    TPC.UpdateObservedPCs();
    auto NewII =
        Corpus.AddToCorpus({Data, Data + Size}, NumNewFeatures, MayDeleteFile,
                           TPC.ObservedFocusFunction(), ForceAddToCorpus,
                           TimeOfUnit, UniqFeatureSetTmp, DFT, II, MD.GetLastMutatedIdex());
  ...
  if (II && FoundUniqFeaturesOfII &&
      ...
      II->U.size() > Size &&
      SM_visited_states == II->VisitedStates) {
...
void Fuzzer::MutateAndTestOne() {
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    ...
    // Partial mutation
    if (!NewSize) {
      NewSize = II.CorrelatedByteIndex + MD.Mutate(CurrentUnitData + II.CorrelatedByteIndex, Size - II.CorrelatedByteIndex, CurrentMaxMutationLen - II.CorrelatedByteIndex);
    }
    ...
    if (StateMachineDebug) {
      Printf("Correlated byte index: %zd, All: %zd\n", II.CorrelatedByteIndex, Size);
      Printf("State Machine after RunOne:\n");
      SM_root->print(0);
      Printf("New state: %d\n", SM_added_states);
      Printf("visited state: ");
      for (auto state : SM_visited_states) {
        Printf("%d ", state->state_name);
      }
      Printf("\n");
    }
    ...
    if (StateMachineDebug) {
      Printf("State Machine after TryDetectingAMemoryLeak:\n");
      SM_root->print(0);
      Printf("New state: %d\n", SM_added_states);
      Printf("visited state: ");
      for (auto state : SM_visited_states) {
    ...
    // StateMachine add-on 
    if (II.VisitedStates == SM_visited_states) {
      II.NumFailedStateTransitions++;
    }
  ...
  // StateMachine add-on
  if (!NewCov) {
    if (II.CorrelatedByteIndex >= 32) {
      II.CorrelatedByteIndex -= 32;
    } else {
