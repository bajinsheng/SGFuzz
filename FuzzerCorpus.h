...
struct InputInfo {
  ...
  size_t NumFailedStateTransitions = 0;
  ...
  // StateMachine add-on 
  Vector<StateMachine*> VisitedStates;
  size_t CorrelatedByteIndex = 0;
  // Power schedule.
...
public:
  ...
  InputInfo *AddToCorpus(const Unit &U, size_t NumFeatures, bool MayDeleteFile,
                         ...
                         const DataFlowTrace &DFT, const InputInfo *BaseII, size_t LastMutatedIdx) {
    ...
    // StateMachine add-on 
    II.VisitedStates = SM_visited_states;

    if (LastMutatedIdx < II.U.size() && SM_added_states)
      II.CorrelatedByteIndex = LastMutatedIdx;
...
private:
  ...
  void UpdateCorpusDistribution(Random &Rand) {
    ...
    // StateMachine add-on 
    size_t sum_hits = SM_root->sum_hits();
    size_t AverageHitCount = 0;
    if (sum_hits > 0) {
      AverageHitCount = (sum_hits - 1)/(SM_num -1);  // exclude root node
    }
    ...
    if (VanillaSchedule) {
      for (size_t i = 0; i < N; i++)
        Weights[i] = (Inputs[i]->NumFeatures || Inputs[i]->VisitedStates.size())
    ...
    if (FeatureDebug) {
      ...
      for (size_t i = 0; i < N; i++)
        Printf("%f ", Weights[i]);
      Printf("Weights\n");
      Printf("%zd Average Hit Count\n", AverageHitCount);
      for (size_t i = 0; i < N; i++)
        Printf("%s ", Sha1ToString(Inputs[i]->Sha1).c_str());
      Printf("Sha1\n");
    }

    // StateMachine Add-on
    // 1. Select the seeds which execute the nodes with fewest hits
    // 2. Assign more energy to the seeds which execute the unstable state sequences

    for (size_t i = 0; i < N; i++) {
      double NumOfRarelyHit = 0;
      for (auto fIndex : Inputs[i]->VisitedStates) {
        if (fIndex->hits < AverageHitCount) NumOfRarelyHit++;
        if (fIndex->hits < 5) NumOfRarelyHit++;
      }

      if (Inputs[i]->VisitedStates.size() || NumOfRarelyHit > 0) {
        Weights[i] += Weights[i] * (double)(NumOfRarelyHit / Inputs[i]->VisitedStates.size());
      }

      if (Inputs[i]->NumExecutedMutations > 0) {
        double MarkovStationary = Inputs[i]->NumFailedStateTransitions > 0 ? Inputs[i]->NumFailedStateTransitions : 1;
        double Level = (double)Inputs[i]->NumExecutedMutations / MarkovStationary;
        Weights[i] *= Min(10.0, Max(1.0, Level));
      }
